...
---CONFIG
------------------------------------------------------------------------
myFont          = "xft:Hack Nerd Font:regular:pixelsize=8"
...
main = do
    ...
    xmproc0 <- spawnPipe "xmobar -x 0 ~/.config/xmonad/xmobar/xmobarrc"
...
--        { ppOutput = \x -> hPutStrLn xmproc0 x  >> hPutStrLn xmproc1 x  >> hPutStrLn xmproc2 x     -- multimonitor
                        { ppOutput = \x -> hPutStrLn xmproc0 x              -- single monitor
...
------------------------------------------------------------------------
myStartupHook = do
          spawnOnce "sh $HOME/.config/xmonad/autostart.sh &"
          setWMName "LG3D"
...
myKeys =
    ...
    -- Scratchpads
        , ("M-C-<Return>", namedScratchpadAction myScratchPads "myTerminal")
...
myManageHook = composeAll
     [
      ...
      , className =? "Virtualbox"   --> doShift "<action=xdotool key super+5>virt</action>"
      , className =? "virt-manager" --> doShift "<action=xdotool key super+5>virt</action>"
...
tall       = renamed [Replace "tall"]     $ limitWindows 12 $ spacing 0 $ ResizableTall 1 (3/100) (1/2) []
grid       = renamed [Replace "grid"]     $ limitWindows 12 $ spacing 0 $ mkToggle (single MIRROR) $ Grid (16/10)
